...
- **Error-channel rule** as a helper pair: `fn rpc_error(id, code, message) -> Value` for envelope faults only (parse, invalid request, unknown method, args-not-an-object, not-initialized, internal bugs) and `fn tool_error(err: &FsmError) -> Value` building `{"isError": true, "content": [{"type": "text", "text": <rendered>}], "structuredContent": {"error": {code, message, path, span?, hint, retryable, details, docs}}}` for every domain error — hosts reliably surface tool results to the model, and the thesis is that errors teach the model the fix.
