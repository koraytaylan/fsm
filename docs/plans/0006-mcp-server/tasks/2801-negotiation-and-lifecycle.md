...
- Error-channel helpers, inline: `rpc_error` produces `{jsonrpc, id, error: {code, message}}` with canonical bytes pinned for one example; `tool_error` produces `isError: true`, one `text` content block, and `structuredContent.error` carrying all eight fields (`code`, `message`, `path`, `span`, `hint`, `retryable`, `details`, `docs`) — field presence asserted by name.
