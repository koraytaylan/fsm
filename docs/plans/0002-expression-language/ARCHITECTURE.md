...
8. The grammar and typing tables below are normative; error codes, spans, and hint contents are part of the behavior under test, not decoration.
...
- `pub fn partial_eval_bool(e: &Expr, ctx: &BTreeMap<String, Val>, scope: &Scope, budget: &mut Budget) -> Truth` — `scope` supplies declared enums and event-field types; the expression is annotated first so decimal `if` widening is present; lazy `and`/`or`/`if` then run in the same walk (unvisited operands and branches are not charged); a selected narrow decimal branch keeps the `if` result scale; remaining `EvtRef` is `Unknown`; `and`/`or`/`not` follow Kleene tables (`False and _ = False`, `True or _ = True`); comparisons and arithmetic containing an `Unknown` operand are `Unknown`; fully-`ctx` subtrees evaluate concretely via `eval`. A concrete sub-evaluation error yields `Unknown` — deliberately conservative: for the enabled-events report an erroring guard is neither definitely enabled nor definitely disabled, and the authoritative loud failure (`run/guard_error`) happens at send time. This choice is documented in SPEC.md.
