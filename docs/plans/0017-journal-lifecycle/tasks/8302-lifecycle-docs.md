...
4. Add to `docs/API-POLICY.md`: store `VERSION` 10, the two new hash domains, the three new format strings, the three new error codes, and — as its own sentence, not a clause — **a 0.3.0 store is not readable by 0.2.x, sealed or not**, because the version stamp moves on first write.
