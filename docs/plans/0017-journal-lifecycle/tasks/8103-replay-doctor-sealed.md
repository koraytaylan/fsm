...
touches:
  ...
  - crates/fsm-cli/tests/fixtures/doctor_sealed.txt
status: planned
merged_as: ""
---
...
3. `doctor` classifies the three new conditions from `verify-from-seal`'s classification rather than reimplementing them: `store/base_missing`, `store/base_mismatch`, and an archive whose manifest does not match its segments.
...
- `crates/fsm-cli/tests/audit_doctor.rs`: a healthy sealed store's doctor output matches `crates/fsm-cli/tests/fixtures/doctor_sealed.txt` byte for byte and reports the cut and the archive id.
