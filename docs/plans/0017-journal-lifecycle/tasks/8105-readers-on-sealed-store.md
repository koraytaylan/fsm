...
touches:
  ...
  - crates/fsm-store/tests/sealed_readers.rs
status: planned
merged_as: ""
---
...
3. `crates/fsm-store/src/snapshot/files.rs` derives the machine and instance sets by scanning for `MachineDefined`, `InstanceCreated`, and `InstanceInvoked`. After a seal those records are archived, so a snapshot written from a sealed store would omit every machine and instance created before the cut and produce a cache claiming a smaller store than exists. **Seed both sets from the base state's own keys**, then add from the live records — the base is exactly the record of what existed at the cut, which is why it is authoritative rather than cached.
...
6. The history page's shape changes, so the `instance_history` tool and the `fsm://instance/{id}/history` resource change with it. Add the seal fields to the **existing** output schema; add no tool. `tools_budget.rs` must still pass, and if the addition does not fit, shorten a description rather than raise the ceiling.
