...
touches:
  ...
  - crates/fsm-cli/src/cli/machine_test.rs
...
1. Add `fsm machine test <machine.json> --cases <cases.json> [--case <name>] [--json]` to the `machine` command table in `crates/fsm-cli/src/cli/machine.rs`. The handler is its own module, `crates/fsm-cli/src/cli/machine_test.rs`: `machine.rs` is already 460 lines against a 1000-line ceiling, and `8502` adds regeneration beside this.
...
8. A definition that fails to compile reports the compiler's findings exactly as `fsm validate` does, before any case runs. With **no catalogue**, since a catalogue lives in a store and this command opens none: a definition that `invoke`s another machine therefore reports the compiler's own findings, exactly as `fsm validate` does when pointed at no data directory. Say so in the module doc.
9. Carry the comparison *rule* into the rendered divergence — `compared as a set`, `compared in order`, `compared by key`. The set-versus-order asymmetry is deliberate and a reader meeting a failing `effects` beside a passing `configuration` asks why first; a report that answers without sending them to the source is what makes the asymmetry safe to have. An author with a broken definition needs the compiler's error, not ten identical case failures.
