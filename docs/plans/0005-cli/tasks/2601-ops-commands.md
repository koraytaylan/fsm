...
3. Add `doctor` (data dir and VERSION, lock holder, snapshot inventory, quick verify summary, effective env) and `repair --truncate-torn-tail` (invokes the plan-0004 repair, printing quarantine path and truncation seq; on any non-torn-tail health it refuses and exits with that health's verify exit code).
...
- `doctor`: renders the data dir path, the `VERSION` value, the lock holder pid while a store handle is held, the snapshot count, the quick verify summary line, and the effective `FSM_DATA_DIR`/`FSM_LOG` values.
