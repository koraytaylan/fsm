...
4. Check the release workflow. If `.github/workflows/release.yml` runs clippy in its verify job, it inherits the same change; if it invokes the `CONTRIBUTING.md` gate by reference, confirm the reference still resolves to the widened command and say which it was in the commit message. (It runs its own copy, and so does `docs/RELEASE.md`, whose note explaining the narrowing must go with it. `crates/fsm-cli/tests/spec_appendix.rs::the_gate_is_the_same_in_ci_release_and_docs` already pins three of those copies to each other and fails until all are updated; `CONTRIBUTING.md` was the fourth copy nothing checked, and joins the pin here.)
