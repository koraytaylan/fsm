...
pub fn validate_event(
    ...
    // A `$`-prefixed name is an engine-generated event (`$done.*`): it is
    // never sendable, whether or not this machine generates it, and it is
    // refused before the unknown-event suggestion could offer a near miss.
    if name.starts_with('$') {
        let mut rejection = reject("req/event_internal", name);
        rejection.message = format!("{name} is generated by the engine, not sent by a caller");
        rejection.hint = format!(
            "send one of the machine's declared events instead: {}",
            sendable_events(m)
        );
    ...
    if ev.internal {
        let mut rejection = reject("req/event_internal", name);
        rejection.message = format!("{name} is declared internal: only the machine raises it");
        rejection.hint = format!(
            "{}; send one of the machine's declared events instead: {}",
            raised_from(m, name),
            sendable_events(m)
...
/// The events a caller may send, for the hint of a refused internal one.
fn sendable_events(m: &CompiledMachine) -> String {
    let names: Vec<&str> = m
        .spec
        .events
        .iter()
