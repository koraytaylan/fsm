...
/// `fsm instance migrate <id> --to <machine>` — move one instance onto a
/// corrected definition, or ask what that would do.
fn migrate(ctx: &mut Ctx, args: &Args) -> u8 {
    if args.positionals.is_empty() {
        return emit_error(
            ctx,
            &ErrorObj::new("args", "instance migrate <id> --to <machine>"),
        );
    }
    let Some(to) = args.flags.get("to").cloned() else {
        return emit_error(
            ctx,
            &ErrorObj::new("args", "to is required").hint("pass --to <machine>"),
        );
    };
    ...
    // A dry run claims no key: it changes nothing, and requiring one would
    // teach a caller to burn keys on questions.
    let mut fields = vec![
        ("instance_id", Value::Str(args.positionals[0].clone())),
        ("to_machine", Value::Str(to)),
    ];
    let rid = if dry_run {
        fields.push(("dry_run", Value::Bool(true)));
        String::new()
    } else {
        match args.flags.get("request-id").cloned() {
            Some(rid) => rid,
            None => match store.allocate_request_id() {
                Ok(rid) => rid,
                Err(error) => return emit_error(ctx, &error),
    ...
    let args_value = Value::Obj(
        fields
            .into_iter()
            .map(|(name, value)| (name.to_string(), value))
            .collect(),
    );
    match crate::mcp::tools::dispatch(
        &mut store,
...
pub static SPECS: &[CmdSpec] = &[
    ...
    CmdSpec {
        path: &["instance", "migrate"],
        positionals: &["instance"],
        flags: &["to", "request-id"],
        switches: &["dry-run"],
        help: "Migrate an instance onto a superseding definition",
        run: migrate,
    },
    CmdSpec {
